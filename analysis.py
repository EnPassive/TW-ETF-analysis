...
def get_disciplined_advice(symbol, current_price, rsi, ma20, period_high, atr, market_regime, trailing_div):
    ...
    if is_dividend_etf and trailing_div > 0:
        ...
        cheap_val = trailing_div / 0.08      # 8% 便宜價
        fair_val = trailing_div / 0.065     # 6.5% 合理價
        expensive_val = trailing_div / 0.05  # 5% 昂貴價
        ...
        if yield_rate >= 8.0:
            ...
            status = "warning"
            buys = [fair_val, cheap_val]
            sells = [current_price * 1.02, current_price + atr]
        else:
            advice = f"🟡 殖利率 {yield_rate:.1f}% (合理區)，耐心等待更低網格"
            status = "neutral"
            buys = [min(current_price * 0.97, fair_val), cheap_val]
            sells = [max(current_price * 1.1, expensive_val)]
    else:
